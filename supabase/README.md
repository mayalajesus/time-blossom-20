...
5. Configure the Auth site URL, redirect URLs, Google provider and SMTP in each
   Supabase project. Invitations are now secure links generated by `/api/data`;
   remove any legacy deployed `invite-member` Function. Acceptance and password
   reset use `/invite/accept`, `/auth/callback` and `/settings`.
